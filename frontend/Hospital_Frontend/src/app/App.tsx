...
export default function App() {
    ...
    horariosApi.listar()
      .then(data => {
        if (Array.isArray(data) && data.length > 0) {
          const mapped: Schedule[] = data.map((h: any) => ({
            ...
            startTime: h.hora_inicio ? h.hora_inicio.substring(0, 5) : '',
            endTime: h.hora_fin ? h.hora_fin.substring(0, 5) : ''
          }));
          setSchedules(mapped);
        }
      })
  ...
  useEffect(() => {
    // Cargar horarios del backend al iniciar
    cargarHorarios();
      ...
      .then(data => {
        if (Array.isArray(data) && data.length > 0) {
          const mapped = data.map((a: any) => ({
            id: String(a.id),
            nombre: a.nombre || '',
            capacidadMaxima: Number(a.capacidadMaxima) || 5,
            ciudad: a.ciudad || 'Tunja',
    ...
    // Cargar usuarios/docentes del backend
    usuariosApi.listar()
      .then(data => {
        if (Array.isArray(data) && data.length > 0) {
          const mapped = data.map((u: any) => ({
            ...
            name: u.name || '',
            cedula: u.cedula || '',
            tipoDocumento: 'C.C.' as const,
            role: u.role || 'docente',
            genero: (u.genero || 'masculino') as 'masculino' | 'femenino',
            password: '',
            permissions: u.permissions || []
          }));
          // Mezclar con usuarios hardcodeados existentes
          setUsers(prev => {
            const cedulasBackend = mapped.map((u: any) => u.cedula);
            const sinDuplicados = prev.filter(u => !cedulasBackend.includes(u.cedula));
            return [...sinDuplicados, ...mapped];
          });
        }
      })
  ...
  const handleAddSchedule = (scheduleData: Omit<Schedule, 'id'>) => {
    // Buscar el id real del estudiante y docente en la BD
    // El frontend manda el id local — necesitamos mapear a id de BD
    const student = students.find(s => s.id === scheduleData.studentId);
    const doctor = users.find(u => u.id === scheduleData.doctorId);

    if (!student || !doctor) {
      toast.error('Estudiante o docente no encontrado');
      return;
    }

    // Preparar datos para el backend
    const payload = {
      doctorId: doctor.cedula,    // usamos cédula para buscar en BD
      studentId: student.cedula,  // usamos cédula para buscar en BD
    ...
    horariosApi.crear(payload)
      .then(resp => {
        if (resp.ok) {
          // Agregar al estado local también para que se vea inmediato
          const newSchedule: Schedule = {
            ...scheduleData,
            id: Date.now().toString()
          };
          setSchedules([...schedules, newSchedule]);
          toast.success('Horario asignado y guardado correctamente');
          // Recargar desde BD para tener ids reales
          cargarHorarios();
        } else {
  ...
  const handleDeleteSchedule = (id: string) => {
    // Intentar eliminar en BD si el id es numérico (viene del backend)
    const idNum = parseInt(id);
    if (!isNaN(idNum)) {
      horariosApi.eliminar(idNum)
        .catch(() => console.log('Error eliminando del backend'));
    }
  ...
  const handleLogin = (documento: string, password: string, role: string) => {
    ...
    if (role === 'estudiante') {
      const student = students.find(s => s.cedula === documento && s.estado === 'ACTIVO');
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 font-medium">Hospital Universitario San Rafael</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
    ...
    <div className="min-h-screen bg-gradient-to-br from-cyan-50 via-sky-50 to-teal-50 flex">
          ...
          {sidebarOpen ? (
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-cyan-100">San Rafael</p>
              </div>
            </div>
          ) : (
            <div className="w-12 h-12 bg-white rounded-xl flex items-center justify-center overflow-hidden mx-auto shadow-lg">
              <span className="text-3xl">🏥</span>
            </div>
          )}
        </div>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <header className="bg-white shadow-md border-b-4 border-cyan-400 px-8 py-5">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold bg-gradient-to-r from-cyan-600 to-teal-500 bg-clip-text text-transparent" style={{ fontFamily: 'Poppins, sans-serif' }}>Hospital Universitario San Rafael</h1>
