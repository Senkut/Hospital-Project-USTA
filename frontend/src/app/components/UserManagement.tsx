              ...
              <div className={`p-4 bg-gray-50 rounded-lg border-2 ${canEditPermissions ? 'border-cyan-200' : 'border-gray-200'} max-h-60 overflow-y-auto`}>
                <div className="grid grid-cols-2 gap-2">
                  {allPermissions.map(permission => (
                    <label
                      key={permission}
                      className={`flex items-center gap-2 p-2 rounded hover:bg-white transition-colors ${canEditPermissions ? 'cursor-pointer' : 'cursor-not-allowed opacity-60'
                        }`}
                    >
                      <input
                        type="checkbox"
            ...
            <div key={user.id} className="bg-white p-4 rounded-lg shadow border border-gray-200">
              <div className="flex justify-between items-start">
                <div className="flex-1 flex items-start gap-3">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white text-xl flex-shrink-0 ${user.genero === 'masculino' ? 'bg-blue-500' : 'bg-pink-500'
                    }`}>
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <span className={`px-3 py-1 rounded-full text-sm font-medium ${user.role === 'director' ? 'bg-purple-100 text-purple-800' :
