...
export function StudentRegistry({ students, onAddStudent, onUpdateStudent }: StudentRegistryProps) {
  ...
  // Cargar estudiantes del backend al montar el componente
  useEffect(() => {
    estudiantesApi.listar()
      .then(data => {
        if (Array.isArray(data) && data.length > 0) {
          // Mapear formato backend → formato frontend
          const mapped = data.map((e: any) => ({
            id: String(e.id),
            cedula: e.cedula,
            name: e.name,
            nombresCompletos: e.name?.split(' ')[0] || '',
            apellidos: e.name?.split(' ').slice(1).join(' ') || '',
            programa: e.programa || '',
            universidad: e.universidad || '',
            institucionEducativa: e.universidad || '',
            tipoVinculacion: 'Estudiante en práctica' as const,
            tipoDocumento: 'C.C.' as const,
            estadoCivil: 'Soltero(a)' as const,
            fechaNacimiento: '',
            lugarNacimiento: '',
            direccionTunja: '',
            lugarResidenciaPermanente: '',
            ...
            celularRepresentanteLegal: '',
            tieneHijos: false,
            genero: (e.genero || 'masculino') as 'masculino' | 'femenino',
            induccionHospitalaria: e.induccionHospitalaria || false,
            fechaInduccion: e.fechaInduccion || '',
            arl: e.arl || false,
            fechaARL: e.fechaARL || '',
            estado: (e.estado || 'ACTIVO') as 'ACTIVO' | 'INACTIVO' | 'PENDIENTE' | 'RETIRADO',
            semestre: String(e.semestre || ''),
            attendanceHistory: []
          }));
          // Llamar al onAddStudent del padre para cada estudiante
          // (esto es temporal hasta conectar el estado global)
          console.log('Estudiantes del backend:', mapped);
        }
      })
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="border-l-4 border-pink-500 pl-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Esposo(a) - Nombre</label>
                  <input
                    type="text"
